...
$k_{j} =$ number of out citations.The system of equations produced from this can be written as follows:\\
  ...
  Calculate the eigenvector of C corresponding to eigenvalue 1, and normalize the eigenvector to 1 to have the ranks.%Geometric interpretation of eigenvector rankings?
  ...
  %Function to ensure nonzero rank? 
