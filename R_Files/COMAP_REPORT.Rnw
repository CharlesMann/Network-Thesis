...
<<echo=FALSE>>=
setwd("C:/Users/Eric Mortberg/Documents/Network-Thesis/R_Files")
...
\vspace{0.1in}
  ...
  Therefore, the column vector $r$ of the ranks of network members is the eigenvector corresponding to eigenvalue $ \lambda = 1$, which C is guarenteed to have as an eigenvalue because C is column stochastic (the column values sum to one in each column). This will be true as long as there are not any dangling nodes or any with zero indegree. \\
